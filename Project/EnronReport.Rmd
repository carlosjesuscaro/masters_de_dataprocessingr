...
# 5. Renaming rvalue to recipient
complete <- complete %>%
              rename(recipient = rvalue)
```

Saving 'complete' and 'employees' data frames
```{r}
saveRDS(complete, file = "complete.rds")
saveRDS(employees, file = "employees.rds")
```
...
#### What is the proportion breakdown of employees based on status?
```{r}
...
It becomes clear that the months of October and November 2001 had a significant number of emails than any other month in the same year.
...
```
By visualizing the data as a pivot table from the sender's status perspective, the highest change in proportion  was from "Employees" sending more emails from September to October from 31.86% to 42.56%.
...
```
```{r}
recipient_summary_proportion  <- complete_2001 %>%
                            group_by(year_month, recipient_status) %>%
                            summarise(count = n(), .groups = 'drop') %>%
                            group_by(year_month) %>%
                            mutate(total_month = sum(count)) %>%
                            mutate(proportion = 100 * count / total_month) %>%
...
knitr::kable(recipient_summary_proportion)
```

Likewise, by having the pivot table of recipients from the employee_status perspective, there were not substantial changes in the proportions between September to October. 
